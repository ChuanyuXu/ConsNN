...
# dataFile = 'data_demo'
dataFile = 'E:\\matlab2018a\\Matlab 2018a\\bin\\博士\\论文\\分层预测\\4.1\\DeleteGansuYunnan\\250610\\data_demo'
...
dataset = TensorDataset(Train_inputs, Train_targets, Train_Constrain)
...
for j in range(epoch__Relaxation):
    for i in range(epoch_learning):
        for batch_inputs, batch_targets, batch_constrain in dataloader:
        # start training
            model_conslearning.train()
            batch_constrain= batch_constrain.to(device)
            batch_inputs = batch_inputs.to(device)
            batch_targets = batch_targets.to(device)
            Cap00=Cap00.to(device)
            Cap_sum = Cap_sum.to(device)
            batch_outputs = model_conslearning(torch.cat((batch_inputs, batch_constrain), dim=1))
            # loss
            loss = loss_fn_SelfCorrecting(batch_outputs, batch_targets, batch_constrain, Cap_sum, Cap00, lamb)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
for i in range(epoch1):
    ...
    for batch_inputs, batch_targets, batch_constrain in dataloader:
        ...
        batch_outputs = model_CRM(batch_inputs,batch_constrain)
        # loss function
        loss=lossMSE(batch_outputs, batch_targets)
        # optimze
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
